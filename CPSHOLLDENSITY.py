...
def inputfunction(anotheriter):
    ...
    if not anotheriter:
        ...
        while True:
            ...
            try:
                ...
                masterfolderpath = input("\nEnter the path of your master folder...\n")
                masterfolderpath = masterfolderpath[1:len(masterfolderpath) - 1]
                masterfolder = [entry.path for entry in os.scandir(masterfolderpath) if "desktop.ini" not in entry.path]

                for file in masterfolder:

                    if "InputFolder" in file:
                ...
                print("\nYour path is invalid: enter with quotes, and ensure you have the full path, with all required folders!...\n")
        ...
        if keycheck in ['y', 'yes', 'yah', 'yeah', 'yup']:
            ...
            inputfiles = [entry.path for entry in inputfiles if keyword in entry.path and "desktop.ini" not in entry.path]
            yeskey = [True, keyword]

        else:

            inputfiles = [entry.path for entry in inputfiles if "desktop.ini" not in entry.path]
            yeskey = [False, False]
        ...
        for file in masterfolder:
            ...
            if Pipepath in file and "cppipe" in file:
        ...
        if not densitybool:
            ...
            skipfiles = input("\nDo you want to see tracing outputs? ('y' or 'n')...\n")

            if skipfiles in ['yes', 'y']:

                print("\ndisplayer on, pls watch for output...\n")
                yeskey.append(False)

            # yeskey is a list that stores [keyword used?, keyword, skipping editing?]
            ...
            # decide whether to skip seeing output images side by side with raw images.
        ...
        inputfiles = [entry.path for entry in os.scandir(inputfolder) if "desktop.ini" not in entry.path]
        filename = input("\nEnter name to save the files as\n")
    ...
    return [inputfiles, inputfolder, Pipepath, outputfolder, correcthresh, integralint, filename, yeskey, densitybool, masterfolderpath]
...
def imagedisplayer(inputfilelist, outputfolder, filepath, yeskey):
    ...
    if yeskey[0]:
        ...
        listfiles = [image.path for image in os.scandir(outputfolder) if yeskey[1] in image.path and "desktop.ini" not in image.path]

    else:

        listfiles = [image.path for image in os.scandir(outputfolder) if "desktop.ini" not in image.path]

    #check whether a key was used. if so, slice for only those images with the key (stored as the second element of yeskey (yeskey[1]).
    ...
    if yeskey[2]:
        ...
        for inputfile in inputfilelist:
            ...
            countr = len(inputfile)

            while inputfile[countr - 1:countr] != "\\":

                countr -= 1

            name = inputfile[countr - 1:len(inputfile)]
            shutil.copy(inputfile, masterfolderpath + "\ImageTraces" + str(name))
            # copy files to imagej folder for sholl analysis

        print("\nskipped image displayer...loading up ImageJ for sholl analysis!...\n")
        filemultiprocessing(masterfolderpath + "\ImageTraces")
        ...
        print("\npress 'g' to mark a 'good' image, and 'f' to flag a 'bad' image\n")

        for image, inputfile in zip(listfiles, inputfilelist):

            refimage = cv.imread(inputfile)
            prosimage = cv.imread(image)
            prosimage = cv.resize(prosimage, (650, 600))
            refimage = cv.resize(refimage, (650, 600))
            cv.imshow('rawimage', refimage)
            cv.moveWindow('rawimage', 0, 0)
            cv.imshow("tracedimage", prosimage)
            cv.moveWindow('tracedimage', 900, 0)
            print("opened" + str(counter))
            pyautogui.click(91, 13)
        ...
        print("\nloading up ImageJ for sholl analysis!...\n")
...
def filemultiprocessing(folder):
    ...
    files = [file.path for file in os.scandir(folder) if "desktop.ini" not in file.path]
    counter = 0
    ...
    for file in files:
        ...
        if __name__ == '__main__':
            ...
            p = multiprocessing.Process(target=shollanalysis, args=(file, counter, files, masterfolderpath))
            p.start()
...
def shollanalysis(file, counter, files, masterfolderpath):
    ...
    ij.IJ.setAutoThreshold(imp, "Moments dark no-reset")
    ...
    while file[countr - 1:countr] != "\\":

        countr -= 1

    name = masterfolderpath + "/ImageTraces" + file[countr-1:len(file)]
    ...
    plugin = sntService.initialize(ij.py.to_imageplus(ij.io().open(name)), True)
    # initialize snt
    ...
    time.sleep(1)
    # orient window
    pyautogui.click(24, 43)
    # click file
    time.sleep(1)
    pyautogui.click(74, 181)
    # click save as
    pyautogui.dragTo(301, 185, .5)
    # drag it to save, so menu is open
    pyautogui.click(301, 185)
    # click on save as
    time.sleep(1)
    pyautogui.tripleClick(321, 494)
    # make sure you click- one click doesn't highlight all text.
    ...
    pyautogui.click(569, 486)
    # click to save

    paths = sntService.getPaths()
    pyautogui.click(24, 43)
    time.sleep(1)
    pyautogui.click(54, 320)
    time.sleep(1)
    # quit
    ...
    try:
        ...
        dia = app["Really Quit?"]
        dia.move_window(0, 506)
        pyautogui.click(99, 600)
        # click to quit
        time.sleep(.5)
        print("time: " + str(time.time() - curenttime) + "s")
...
def dataorganizer(boolredo = False):
    ...
    while True:
        time.sleep(5)
        print("\nTRACING COMPLETE. Waiting for user to run bulk sholl analysis...\n")
        time.sleep(5)
        print("\nImageJ->Plugins->Neuroanatomy->SNT->Scripts->Bulk->Bulk Sholl Analysis...\n")

        try:

            pd.read_csv(masterfolderpath + "\ShollCell_Output\_Sholl_Metrics.csv")
            time.sleep(10)
            print("\nFound Bulk Sholl File. Proceeding with downstream analysis...\n")
            break
    ...
    shollsheet = pd.read_csv(outputfolder + "\_Sholl_Metrics.csv")
    ...
    if redobool in ['y', 'h', 'happy', 'yes']:
        ...
        shollsheet.to_excel(masterfolderpath + "\AnalysisOutput" + "\\" + "sholl_spliced" + str(microglialdataframename), index=False)
        cellprofilersheet.to_excel(masterfolderpath + "\AnalysisOutput" + "\\" + "cellp_spliced" + str(microglialdataframename), index=False)
        # save sliced sheets
...
def downstream(cellprofilersheet, shollsheet):
    ...
    print(str(len(set(PLXsheet_sholl['FileName']))) + " Transplant files")
    print(str(len(set(Controlsheet_sholl['FileName']))) + " Control files")
    ...
    Celldf.to_excel(masterfolderpath + "\AnalysisOutput" + "\cellp_analyzed.xlsx", index=False)
    Sholldf.to_excel(masterfolderpath + "\AnalysisOutput" + "\sholl_analyzed.xlsx", index=False)
    # to excel
    ...
    for column in [column for column in Celldf.columns[1:] if column not in cnonsignificantnames]:
        ...
        for value in Celldf[column]:
            ...
            if index == 0:
                ...
                print(str(column) + " difference for Transplant-Control: " + operator + str(subvalue - value))
                break
...
def imageJtcoordtracer(file):
    ...
    pyautogui.rightClick(143,914)
    pyautogui.click(181, 878)
    pyautogui.click(screenWidth//2, 165)
    ...
    while counter < 4:
        ...
        pyautogui.write(masterfolderpath + "\DensityCoords" + "\\" + name + ".csv")
        pyautogui.press("enter")
        #save filename
        time.sleep(.1)
        app = Application().connect(title_re=filename, found_index=0)
...
def densitycounter(reffilename, cellprofilersheet):
    ...
    densityfiles = [file.path for file in os.scandir(masterfolderpath + "\DensityCoords") if reffilename in file.path and "desktop.ini" not in file.path]
    ...
    for key in HippocampusDictionary.keys():
        ...
        pd.DataFrame(curposdictionary).to_excel(masterfolderpath + "\DensityOutput" + "\\" + str(key) + "_" + str(avgmed) + "_" + "HippocampalDistributiton" + ".xlsx", index=False)
    ...
    for key in NeocortexDictionary.keys():
        ...
        pd.DataFrame(curposdictionary).to_excel(masterfolderpath + "\DensityOutput" + "\\" + str(key) + "_" + str(avgmed) + "_" + "NeocorticalDistribution" + ".xlsx", index=False)
...
def main(x=False):
    ...
    # global variables are not passed to process instances
