    ...
    if not anotheriter:
        ...
        while True:
            ...
            try:
                ...
                masterfolder = [entry.path for entry in os.scandir(masterfolderpath)]
            ...
            if counter >= len(files)-1:
                ...
                while True:
                    ...
                    try:
                        ...
                        pd.read_csv(outputfolder + "\_Sholl_Metrics.csv")
                        break
...
def shollanalysis(file, counter, files, masterfolderpath):
    ...
    time.sleep(1)
    # orient window
    pyautogui.click(28, 42)
    # click file
    time.sleep(1)
    pyautogui.click(72, 184)
    # click save as
    pyautogui.dragTo(304, 184, .5)
    # drag it to save, so menu is open
    pyautogui.click(304, 184)
    # click on save as
    time.sleep(3)
    pyautogui.tripleClick(324, 490)
    ...
    if counter >= len(files)-1:
        ...
        time.sleep(2)
        print("nearly done")
        pyautogui.click(167, 41)
        time.sleep(1)
        pyautogui.click(258, 83)
        time.sleep(1)
        pyautogui.dragTo(434, 87)
        time.sleep(1)
        pyautogui.click(417, 240)
    ...
    pyautogui.click(26, 48)
    time.sleep(1)
    pyautogui.click(51, 323)
    time.sleep(1)
    # quit
...
def dataorganizer(boolredo = False):
    ...
    if redobool in ['y', 'h', 'happy', 'yes']:
        ...
        shollsheet.to_excel("D:\Karan\Training\ShollCell_Output" + "\\" + "sholl_spliced" + str(microglialdataframename), index=False)
        cellprofilersheet.to_excel("D:\Karan\Training\ShollCell_Output" + "\\" + "cellp_spliced" + str(microglialdataframename), index=False)
        # save sliced sheets
...
def downstream(cellprofilersheet, shollsheet):
    ...
    Celldf.to_excel("D:/Karan/Training/AnalysisOutput" + "/" + "cellp_analyzed.xlsx", index=False)
    Sholldf.to_excel("D:/Karan/Training/AnalysisOutput" + "/" + "sholl_analyzed.xlsx", index=False)
    # to excel
...
def imageJtcoordtracer(file):
    ...
    ij.ui().show(imp)
    #show image
    dapp.move_window(0, screenHeight-150)
    time.sleep(.1)
    ...
    pyautogui.rightClick(144,964)
    pyautogui.click(195, 925)
    pyautogui.click(709, 442)
    ...
    while counter < 4:
        ...
        pyautogui.write("D:\\Karan\\Training\\DensityCoords\\" + name + ".csv")
        pyautogui.press("enter")
        #save filename
        time.sleep(.1)
        app = Application().connect(title_re=filename, found_index=0)
...
def densitypreprocessing(cellprofilerfile):
    ...
    cellprofilersheet.to_excel(outputfolder + "/" + "cellp_spliced" + str(microglialdataframename), index=False)
    # save file
...
def densitycounter(reffilename, cellprofilersheet):
    ...
    densityfiles = [file.path for file in os.scandir("D:\Karan\Training\DensityCoords") if reffilename in file.path]
    ...
    for key in HippocampusDictionary.keys():
        ...
        pd.DataFrame(curposdictionary).to_excel("D:/Karan/Training/DensityOutput" + "/" + str(key) + "_" + str(avgmed) + "_" + "HippocampalDistributiton" + ".xlsx", index=False)
    ...
    for key in NeocortexDictionary.keys():
        ...
        pd.DataFrame(curposdictionary).to_excel("D:/Karan/Training/DensityOutput" + "/" + str(key) + "_" + str(avgmed) + "_" + "NeocorticalDistribution" + ".xlsx", index=False)
